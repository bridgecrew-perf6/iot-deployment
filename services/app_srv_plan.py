...
def provision(
    ...
    if app_srv_plan_name not in {
        desc_list_res.name
        for desc_list_res in website_client.app_service_plans.list_by_resource_group(
            resource_group_name
        )
    }:
        app_sku_info = SkuDescription(
        ...
        poller = website_client.app_service_plans.begin_create_or_update(
