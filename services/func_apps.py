...
class Provisioner:
    ...
    def _code_to_copy(self, org_func_app_name: str, postfix: Optional[str] = None) -> str:
        ...
        func_app_code_path = os.path.join(self.functions_code_path, org_func_app_name)
        func_app_copy_path = os.path.join(self.functions_copy_path, func_app_name)
        shutil.copytree(func_app_code_path, func_app_copy_path)
        return func_app_name

    def _repo_init(self):
        basename, dirname = os.path.split(self.functions_code_path)
        self.functions_copy_path = os.path.join(basename, f"{dirname}-copy")
        # Remove if there is a folder with the same name.
        if os.path.isdir(self.functions_copy_path):
            shutil.rmtree(self.functions_copy_path)
        # git init
        self.repo = Repo.init(self.functions_copy_path)
                ...
                "bindings": [
                    {
                        ...
                        "connectionStringSetting": "{}{}".format(self.cosmosdb_name, functions.COSMOSDB_CONN_STR_POSTFIX),
            ...
            "bindings": [
                {
                    ...
                    "connection": "{}{}".format(self.iot_hub_name, functions.IOT_HUB_CONN_STR_POSTFIX),
                    "cardinality": "many",
    ...
    def _configure_func_app(self, func_app_name: str, func_conf: Dict, credentials: Optional[Dict] = None):
        func_app_path = os.path.join(self.functions_copy_path, func_app_name)
    ...
    def _copy_all_other(self):
        for item_name in os.listdir(self.functions_code_path):
            item_path = os.path.join(self.functions_code_path, item_name)
            if os.path.isfile(item_path):
                shutil.copy2(item_path, self.functions_copy_path)
    ...
    def _repo_deploy(self):
        ...
        poller = self.website_client.web_apps.begin_list_publishing_credentials(self.resource_group_name, self.functions_name)
        user = poller.result()
    ...
    def _cleanup(self):
        self.repo.close()
        for root, dirs, files in os.walk(self.functions_copy_path):
            for dir in dirs:
        ...
        try:
            ...
            self.logger.warning("Could not delete '{}' Please erase manually!".format(self.functions_copy_path))
